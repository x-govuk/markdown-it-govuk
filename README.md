...
| `govspeak`   | `boolean \| Array` | Enable support for Govspeak extensions. Set this option to `true` to enable all supported extensions, or array containing individual extensions to include (choose from `example-callout` and `information-callout`). Default is `false`.                                                        |
