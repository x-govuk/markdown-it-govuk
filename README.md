...
| `govspeak`          | `boolean \| Array` | Enable support for Govspeak extensions. Set this option to `true` to enable all supported extensions, or provide an array containing individual extensions to include (choose from `blockquote`, `example-callout`, `information-callout` and `warning-callout`). Default is `false`.                 |
